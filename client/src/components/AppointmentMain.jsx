                  ...
                  .map((a, index) => (
                    <tr key={index} className="text-gray-800 lg:h-18">
                      ...
                      <td className="py-2 px-3 border-b border-gray-200 text-left">
                        <button className={`w-23 h-9 ${a.status==='not booked'?'bg-[#BFA6F2]':a.status==='booked'?'bg-[#9d75eb]':a.status==='visited'?'bg-[#814de5]':''} text-sm text-white font-bold rounded-2xl cursor-pointer`}>
                          {a.status}
                        </button>
                      </td>
